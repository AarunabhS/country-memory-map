...
(function (root, factory) {
  ...
  class ProfileManager {
    ...
    applySessionEnd(profileId, mode, session) {
      ...
      if (['complete', 'completed', 'time', 'lives'].includes(session.endReason)) { stats.gamesCompleted += 1; modeStats.gamesCompleted = Number(modeStats.gamesCompleted || 0) + 1; }
  ...
  class GameTracker {
    ...
    onEvent(event, state, config) {
      ...
      if (event.type === 'hint') this.session.assisted = true;
