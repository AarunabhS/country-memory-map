  ...
  function boot() {
    ...
    app.insertAdjacentHTML('afterbegin', `
      ...
      <section class="setup-panel" aria-label="Choose a game">
        ...
        <details class="rules-details"><summary>Scoring & rules</summary><p>New answer: 100 points × speed bonus × streak bonus, rounded. Under 2s: ×1.30; under 4s: ×1.20; up to 7s: ×1.10. Streaks of 5 / 10 / 20 / 40 earn ×1.05 / ×1.10 / ×1.20 / ×1.30.</p><p>World Conquest: duplicates change nothing. Invalid submissions reset your streak. Other games: 10 seconds per question; wrong attempts cost 25 then 50 points. A third mistake or timeout reveals the answer. Accuracy counts submitted attempts and timeouts. Difficulty starts with editorial tiers, separately for capitals and map locations.</p></details>
    ...
    function showResults(r) {
      ...
      const metrics = [[r.config.family==='conquest'?'Countries found':'Correct answers',`${r.correct} / ${r.total}`],['Final score',r.score.toLocaleString()],['Accuracy',`${r.accuracy}%`],['Best streak',r.bestStreak],[r.completed?'Completion time':'Time played',formatTime(r.elapsedTime)],['Average answer',r.averageResponseTime===null?'—':`${r.averageResponseTime.toFixed(1)}s`],['Fastest answer',r.fastestAnswer===null?'—':`${r.fastestAnswer.toFixed(1)}s`],['Wrong / timed out',r.wrong]];
