...
| Country entry/selection | **AUTOMATED VERIFIED** — engine/data/server answers plus both adapter callback identities, Google one-event delivery, mode policy, single-flight, cancellation, structured bridge submission, and small-target sizing | **MANUALLY VERIFIED WITH EVIDENCE** for local typed Countries/Capitals and retained small-target selection; **MANUAL VERIFICATION REQUIRED** for physical touch and production Google | Local browser: consecutive India/Brazil marks, New Delhi capital mark, and Mauritius selection through its invisible target; repeat with physical coarse-touch hardware and production Google |
...
| Free map | **AUTOMATED VERIFIED** — direct structured bridge preparation/submission contracts plus data contracts | **MANUALLY VERIFIED WITH EVIDENCE** for local typed Countries/Capitals; **MANUAL VERIFICATION REQUIRED** for microphone input and the full retained matrix | Local browser verified India then Brazil without stale feedback and New Delhi in Capitals; retained labels, filters, physical microphone, and reset remain open |
