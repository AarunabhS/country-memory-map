...
(function (root, factory) {
  ...
  class Engine {
    constructor(countries, { now = () => Date.now(), random = Math.random, onEvent = () => {} } = {}) {
      ...
      this.now = now; this.random = random; this.onEvent = onEvent; this.state = { gameStatus: 'idle' };
    }
    ...
    start(config) {
      ...
      this.state = { mode: `${config.family}:${config.variant}`, score: 0, streak: 0, bestStreak: 0,
        ...
        questionNumber: 0, questionLimit: practice ? this.pool.length : rules.questions ?? null, personalBest: null };
    ...
    submitText(raw) {
      ...
      if (this.config.family === 'conquest') {
        ...
        const at = this.now(), seconds = Math.max(0, (at - s.lastCorrectAt) / 1000);
    ...
    resolveQuestion(correct, at, reason) {
      ...
      q.resolved = true;
      const seconds = Math.max(0, (at - s.questionStartedAt) / 1000);
  ...
  return { TYPES, MODES, Engine, LocalProfile, calculateScore, normalize, matchesDifficulty };
});
