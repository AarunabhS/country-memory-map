...
import {makeRoom,joinRoom,roomAction,syncRoom,publicRoom,ranking,DATA,createChallenge,MAX_PLAYERS} from '../room-engine.mjs';
...
test('2-9 players, duplicate and Unicode names, host-only settings',()=>{const r=room();for(let i=1;i<MAX_PLAYERS;i++)joinRoom(r,{id:'p'+i,tokenHash:'t'+i,name:'Player '+i},base);assert.equal(r.players.length,9);assert.throws(()=>joinRoom(r,{id:'tenth',name:'Ten'},base),/9 players/);const n=room();joinRoom(n,{id:'hindi',name:'अरुण'},base);joinRoom(n,{id:'bangla',name:'অরুণ'},base);assert.throws(()=>joinRoom(n,{id:'dupe',name:'ARUN'},base),/already in use/);assert.throws(()=>roomAction(n,n.players[1],{action:'settings'},base),/host/);});
...
test('room question timing is configurable and enforced',()=>{const r=room();joinRoom(r,{id:'guest',name:'Guest'},base);roomAction(r,r.players[0],{action:'settings',config:{family:'find',variant:'standard',questionTime:30}},base);roomAction(r,r.players[1],{action:'ready',ready:true},base);roomAction(r,r.players[0],{action:'start'},base);assert.equal(r.config.questionTime,30);assert.equal(r.players[0].game.state.currentQuestion.timeLimit,30);assert.equal(r.players[0].game.state.currentQuestion.deadline,base+33000);});
...
test('a host and eight friends start together with independent gameplay',()=>{const r=room();r.config={family:'capital',variant:'classic',difficulty:'medium',region:'Africa',questionCount:10,questionTime:20};for(let i=1;i<MAX_PLAYERS;i++){joinRoom(r,{id:'friend-'+i,tokenHash:'token-'+i,name:'Friend '+i},base);roomAction(r,r.players[i],{action:'ready',ready:true},base);}roomAction(r,r.players[0],{action:'start'},base);assert.equal(r.players.length,9);assert.ok(r.players.every(p=>p.game));assert.ok(r.players.every(p=>p.game.state.currentQuestion.id===r.players[0].game.state.currentQuestion.id));syncRoom(r,base+3000);for(const p of r.players){const q=p.game.state.currentQuestion,c=DATA.find(c=>c.country_id===q.countryId);roomAction(r,p,{action:'answer',kind:'text',value:c.capital[0].name,questionId:q.id,matchId:r.match.id,seq:1,responseTime:1},base+4000);}const view=publicRoom(r,r.players[0],base+4000);assert.equal(view.maxPlayers,9);assert.equal(view.standings.length,9);assert.ok(view.standings.every(p=>p.correct===1));});
...
test('atomic parallel joins allow eight friends but never a tenth member',async()=>{const store=new MemoryStore(),r=room();await store.insert(r);const responses=await Promise.all(Array.from({length:12},(_,i)=>handle(new Request('http://test/rooms/GEOABCDEF',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'join',name:'Guest '+i})}),store,base+1000)));assert.equal(responses.filter(r=>r.ok).length,8);assert.equal(JSON.parse((await store.get(r.code)).body).players.length,9);});
...
test('feedback belongs to its question and is cleared before rematch',()=>{
 const r=room();r.config={family:'find',variant:'standard',difficulty:'easy',questionCount:10,questionTime:10};ready(r);const p=r.players[0];const first=p.game.state.currentQuestion.id;
