...
- **Map and geometry:** the world map is rendered by the existing `GameMap` adapter in `game-map.js`, using the embedded Natural Earth 1:10m country geometry in `countries-data.js` / `countries.geojson`. `country-borders.js` provides the derived adjacency data used by map gameplay.
