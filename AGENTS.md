...
- Treat Google Maps 3D and map/globe integration as protected shared subsystems. Never hard-code new keys or secrets. Browser-delivered Maps API keys are distinct from private/server secrets and must use the minimum required API permissions, authorized website/referrer restrictions, and monitoring/quota controls where appropriate. The root local-globe fallback and retained singleton game/checker bridge are approved in ADR 0002; once production-verified, preserve that fallback contract in future work.
